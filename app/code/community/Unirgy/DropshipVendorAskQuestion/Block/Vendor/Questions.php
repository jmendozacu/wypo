...
class Unirgy_DropshipVendorAskQuestion_Block_Vendor_Questions extends Mage_Core_Block_Template
{
    ...
    protected function _applyRequestFilters($collection)
    {
        ...
        if (!is_null($param) && $param!=='') {
            if (Mage::helper('udropship')->compareMageVer('1.9.1.1', '1.14.1.0')) {
                if ($param) {
                    $collection->addFieldToFilter('LENGTH(answer_text)', array('gt' => 0));
                } else {
                    $collection->addFieldToFilter(array('answer_text','LENGTH(answer_text)'), array(array('null' => 1), array('eq' => 0)));
                }
            } else {
                if ($param) {
