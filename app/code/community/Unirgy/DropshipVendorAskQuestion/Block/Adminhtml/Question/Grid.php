...
class Unirgy_DropshipVendorAskQuestion_Block_Adminhtml_Question_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _prepareColumns()
    {
        ...
        $this->addColumn($prefix.'question_date', array(
            'header'        => Mage::helper('udropship')->__('Question Date'),
            'align'         => 'left',
            'type'          => 'datetime',
            'width'         => '100px',
            'index'         => 'question_date',
        ...
        $this->addColumn($prefix.'question_status', array(
            'header'        => Mage::helper('udropship')->__('Question Status'),
            'align'         => 'left',
            'type'          => 'options',
            'options'       => $statuses,
            'width'         => '100px',
        ...
        $this->addColumn($prefix.'customer_name', array(
            'header'        => Mage::helper('udropship')->__('Customer Name'),
            'align'         => 'left',
            'width'         => '100px',
            'index'         => 'customer_name',
        ...
        $this->addColumn($prefix.'question_text', array(
            'header'        => Mage::helper('udropship')->__('Question Text'),
            'align'         => 'left',
            'index'         => 'question_text',
            'type'          => 'text',
            'truncate'      => 50,
        ...
        $this->addColumn($prefix.'vendor_id', array(
            ...
            'format'        => sprintf('<a onclick="this.target=\'blank\'" href="%sid/$vendor_id/">$vendor_name</a>', $this->getUrl('adminhtml/udropshipadmin_vendor/edit'))
        ));
        }

        $this->addColumn($prefix.'answer_date', array(
            'header'        => Mage::helper('udropship')->__('Answer Date'),
            'align'         => 'left',
            'type'          => 'datetime',
            'width'         => '100px',
            'index'         => 'answer_date',
        ...
        $this->addColumn($prefix.'answer_status', array(
            'header'        => Mage::helper('udropship')->__('Answer Status'),
            'align'         => 'left',
            'type'          => 'options',
            'options'       => $statuses,
            'width'         => '100px',
        ...
        $this->addColumn($prefix.'answer_text', array(
            'header'        => Mage::helper('udropship')->__('Answer Text'),
            'align'         => 'left',
            'index'         => 'answer_text',
            'type'          => 'text',
            'truncate'      => 50,
        ...
        $this->addColumn($prefix.'context', array(
            'header'        => Mage::helper('udropship')->__('Context'),
            'align'         => 'left',
            'index'         => 'increment_id',
        ...
        $this->addColumn($prefix.'action',
            array(
                ...
                'actions'   => array(
                    array(
                        'caption' => Mage::helper('udropship')->__('Edit'),
                        'url'     => array(
                            'base'=>'adminhtml/udqaadmin_index/edit',
                            'params'=> array(
    ...
    protected function _prepareMassaction()
    {
        if ($this->uIsMassactionAvailable()) {
            ...
            $this->getMassactionBlock()->addItem('delete', array(
                ...
                'url'  => $this->getUrl('adminhtml/udqaadmin_index/massDelete', array('ret' => Mage::registry('usePendingFilter') ? 'pending' : 'index')),
            ...
            $this->getMassactionBlock()->addItem('update_question_status', array(
                'label'         => Mage::helper('udropship')->__('Update Question Status'),
                'url'           => $this->getUrl('adminhtml/udqaadmin_index/massUpdateQuestionStatus', array('ret' => Mage::registry('usePendingFilter') ? 'pending' : 'index')),
                'additional'    => array(
                    'status'    => array(
                        'name'      => 'status',
                        'type'      => 'select',
            ...
            $this->getMassactionBlock()->addItem('update_answer_status', array(
                'label'         => Mage::helper('udropship')->__('Update Answer Status'),
                'url'           => $this->getUrl('adminhtml/udqaadmin_index/massUpdateAnswerStatus', array('ret' => Mage::registry('usePendingFilter') ? 'pending' : 'index')),
                'additional'    => array(
                    'status'    => array(
                        'name'      => 'status',
                        'type'      => 'select',
            ...
            $this->getMassactionBlock()->addItem('send_customer_notification', array(
                'label'         => Mage::helper('udropship')->__('Send Customer Notification'),
                'url'           => $this->getUrl('adminhtml/udqaadmin_index/massSendCustomer', array('ret' => Mage::registry('usePendingFilter') ? 'pending' : 'index')),
            ));
            $this->getMassactionBlock()->addItem('send_vendor_notification', array(
                'label'         => Mage::helper('udropship')->__('Send Vendor Notification'),
                'url'           => $this->getUrl('adminhtml/udqaadmin_index/massSendVendor', array('ret' => Mage::registry('usePendingFilter') ? 'pending' : 'index')),
            ));
        }
    }
    ...
    {
        return $this->getUrl('adminhtml/udqaadmin_index/edit', array(
            'id' => $row->getId(),
            'vendorId' => $this->getVendorId(),
            'customerId' => $this->getCustomerId(),
            'ret'       => ( Mage::registry('usePendingFilter') ) ? 'pending' : null,
    ...
    public function getGridUrl()
    {
        if( $this->getVendorId() || $this->getCustomerId() ) {
            return $this->getUrl('adminhtml/udqaadmin_index/' . (Mage::registry('usePendingFilter') ? 'pending' : ''), array(
                'vendorId' => $this->getVendorId(),
                'customerId' => $this->getCustomerId(),
            ));
        } else {
