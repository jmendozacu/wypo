...
class Unirgy_DropshipVendorAskQuestion_Udqaadmin_IndexController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function indexAction()
    {
        $this->_title(Mage::helper('udropship')->__('Sales'))
             ...
             ->_title(Mage::helper('udropship')->__('Vendor Questions'))
             ->_title(Mage::helper('udropship')->__('All Questions/Answers'));
    ...
    public function pendingAction()
    {
        $this->_title(Mage::helper('udropship')->__('Sales'))
             ->_title(Mage::helper('udropship')->__('Dropship'))
             ->_title(Mage::helper('udropship')->__('Vendor Questions'))
             ->_title(Mage::helper('udropship')->__('Pending Questions/Answers'));
    ...
    public function editAction()
    {
        $this->_title(Mage::helper('udropship')->__('Sales'))
             ->_title(Mage::helper('udropship')->__('Dropship'))
             ->_title(Mage::helper('udropship')->__('Vendor Questions'))
             ->_title(Mage::helper('udropship')->__('Edit Vendor Question'));
    ...
    public function saveAction()
    {
        if (($data = $this->getRequest()->getPost()) && ($questionId = $this->getRequest()->getParam('id'))) {
            ...
            if (! $question->getId()) {
                Mage::getSingleton('adminhtml/session')->addError(Mage::helper('udropship')->__('The question was removed by another user or does not exist.'));
            } else {
                try {
                    $question->setIsAdminChanges(true);
                    $question->addData($data)->save();
                    Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('udropship')->__('The question has been saved.'));
                } catch (Exception $e){
    ...
    public function deleteAction()
    {
        ...
        try {
            ...
            Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('udropship')->__('The question has been deleted'));
    ...
    public function massDeleteAction()
    {
        ...
        if(!is_array($questionsIds)) {
             Mage::getSingleton('adminhtml/session')->addError(Mage::helper('udropship')->__('Please select question(s).'));
        } else {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('udropship')->__('Total of %d record(s) have been deleted.', count($questionsIds))
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
            }
    ...
    public function massUpdateQuestionStatusAction()
    {
        ...
        if(!is_array($questionsIds)) {
            ...
            try {
                ...
                $session->addSuccess(
                    Mage::helper('udropship')->__('Total of %d record(s) have been updated.', count($questionsIds))
                );
            }
                ...
                $session->addError(Mage::helper('udropship')->__('An error occurred while updating the selected question(s).'));
            }
        }
    ...
    public function massSendCustomerAction()
    {
        ...
        if(!is_array($questionsIds)) {
            ...
            try {
                ...
                $session->addSuccess(
                    Mage::helper('udropship')->__('Total of %d customer notification(s) have been sent.', count($questionsIds))
                );
            }
            catch (Mage_Core_Exception $e) {
                $session->addError($e->getMessage());
                ...
                $session->addError(Mage::helper('udropship')->__('An error occurred while sending customer notification(s).'));
            }
        }
    ...
    public function massSendVendorAction()
    {
        ...
        if(!is_array($questionsIds)) {
            ...
            try {
                ...
                $session->addSuccess(
                    Mage::helper('udropship')->__('Total of %d vendor notification(s) have been sent.', count($questionsIds))
                );
            }
            catch (Mage_Core_Exception $e) {
                $session->addError($e->getMessage());
                ...
                die("$e");
                $session->addError(Mage::helper('udropship')->__('An error occurred while sending vendor notification(s).'));
            }
        }
    ...
    protected function _isAllowed()
    {
        switch ($this->getRequest()->getActionName()) {
            case 'pending':
                return Mage::getSingleton('admin/session')->isAllowed('sales/udropship/udqa/question_pending');
                break;
            default:
                return Mage::getSingleton('admin/session')->isAllowed('sales/udropship/udqa/question_all');
                break;
        }
    }
}
