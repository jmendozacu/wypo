    ...
    extends Unirgy_DropshipMulti_Model_StockIndexer_EE11300_Default
{
    ...
    protected function _syncUdmWithStockItem($entityIds)
    {
        ...
        $select->columns(array('item_id' => 'cisi.item_id', 'product_id' => 'cisi.product_id', 'stock_id' => 'cisi.stock_id', 'qty' => new Zend_Db_Expr('0'), 'is_in_stock' => $statusExpr));

        if (!is_null($entityIds)) {
            $select->where('e.entity_id IN(?)', $entityIds);
        }

        $query = $select->insertFromSelect($this->getTable('cataloginventory/stock_item'), array('item_id', 'product_id', 'stock_id', 'qty', 'is_in_stock'), true);
        $adapter->query($query);
