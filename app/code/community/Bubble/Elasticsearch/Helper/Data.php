...
class Bubble_Elasticsearch_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function getProductCollection($queryText, $store = null)
    {
		...
		$vendor_product_ids = $readConnection->fetchCol('SELECT product_id FROM '.$udropship_vendor_product_table.' WHERE vendor_sku like "%'.addslashes($queryText).'%"');
		if(count($vendor_product_ids)>0){		
			$productIds = array_merge($vendor_product_ids,$productIds);
