 ...
 */
class Unirgy_DropshipSellYours_Model_Rewrite1901_CatalogSearch_Resource_Fulltext extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
        ...
        while (true) {
            ...
            foreach ($products as $productData) {
                ...
                */
                ...
                $udsellStatus[$productData['entity_id']] = !(
                    !isset($productAttr[$visibility->getId()]) || !in_array($productAttr[$visibility->getId()], $allowedVisibilityValues)
                    || !isset($productAttr[$status->getId()]) || !in_array($productAttr[$status->getId()], $statusVals)
                );
            }

            //$this->_saveProductIndexes($storeId, $productIndexes);
            $this->_udsellSaveProductIndexes($storeId, $productIndexes, $udsellStatus);
        }
    ...
    public function prepareResult($object, $queryText, $query)
    {
        ...
        if (!$query->getIsProcessed()) {
            ...
            $select->columns(array('s.udsell_status'));
    ...
    protected function _udsellSaveProductIndexes($storeId, $productIndexes, $udsellStatus)
    {
        if ($this->_engine) {
            $this->_engine->udsellSaveEntityIndexes($storeId, $productIndexes, $udsellStatus);
        }
        return $this;
    }
