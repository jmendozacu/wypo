...
class Unirgy_DropshipSplit_Block_Onepage_Shipping extends Mage_Checkout_Block_Onepage_Shipping_Method_Available
{
...
EOT;
            ...
            if (Mage::getStoreConfigFlag('carriers/udsplit/sm_hide_amounts')) {
                ...
                $qtyLbl = Mage::helper('udropship')->__('Qty');
                $html .= <<<EOT
<script type="text/javascript">
...
EOT;
            ...
            if (Mage::helper('udropship')->isModuleActive('Idev_OneStepCheckout')) {
                $oscUrl = $this->getUrl('onestepcheckout/ajax/set_methods_separate', array('_secure'=>true));
                $oscUpdatePayments = $this->settings['enable_update_payment_on_shipping'] ? 'true' : 'false';
                $html .= <<<EOT
<script>
    var url = '$oscUrl';
    var update_payments = $oscUpdatePayments;
    $$('select.shipment-methods').invoke('observe', 'change', get_separate_save_methods_function(url, update_payments));
</script>
EOT;
            }
