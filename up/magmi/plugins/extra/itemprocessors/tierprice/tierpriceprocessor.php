    ...
    public function processItemAfterId(&$item, $params = null)
    {
		...
		if($item['udropship_vendor']==''){
			...
			foreach ($tpcol as $k) {
				...
				$cgid = $tpinf["id"];
				// add tier price
				$sql = "INSERT INTO $_custom_vendor_tier_price_table
				(vendor_product_id,product_id,vendor_id,all_groups,customer_group_id,qty,value,website_id) VALUES ";
				$inserts = array();
				$data = array();
				
				$vendor_product_find_sql = "SELECT * FROM " . $this->tablename("udropship_vendor_product") . " where vendor_id=".$item['udropship_vendor']." AND product_id=".$pid;$stmt = $this->select($vendor_product_find_sql);
				$vendor_product_id = '';
				while ($result = $stmt->fetch()) {				
					$vendor_product_id = $result['vendor_product_id'];
					break;
				}	
				if($vendor_product_id==''){
					$this->log("Product ID# ".$pid.": not associated with vendor# ".$item['udropship_vendor'],"error");
					continue;
				}
				if ($item[$k] == "") {
					continue;
				...
				if (count($inserts) > 0) {
					$sql .= implode(",", $inserts);
					$sql .= " ON DUPLICATE KEY UPDATE `value`=VALUES(`value`),`qty`=VALUES(`qty`)";					
					$this->insert($sql, $data);
				}
			}			
